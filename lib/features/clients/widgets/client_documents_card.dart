...
import '../../../providers/client_document_provider.dart';

class ClientDocumentsCard extends ConsumerWidget {
  final Client client;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final documents = ref.watch(documentsByClientProvider(client.id));

    final categories = {
      'Bescheide & Bewilligungen': (Symbols.gavel, Colors.blue),
      'Berichte & Dokumentation': (Symbols.summarize, Colors.green),
      'Teilhabeplanung': (Symbols.track_changes, Colors.orange),
  ...
  Widget _buildDocumentItem(BuildContext context, WidgetRef ref, ClientDocument document, Color categoryColor) {
    return Container(
  ...
  void _showUploadDialog(BuildContext context, WidgetRef ref) {
    ...
    String selectedCategory = 'Bescheide & Bewilligungen';
              ...
              children: [
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText: 'z.B. Kostenzusage 2026',
                  ),
                ),
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    DropdownMenuItem(value: 'Bescheide & Bewilligungen', child: Text('Bescheide & Bewilligungen')),
                    DropdownMenuItem(value: 'Berichte & Dokumentation', child: Text('Berichte & Dokumentation')),
                    DropdownMenuItem(value: 'Teilhabeplanung', child: Text('Teilhabeplanung')),
            ...
            FilledButton.icon(
              onPressed: () async {
                ...
                if (result != null && result.files.single.path != null) {
                  ...
                  final doc = await ref.read(clientDocumentsProvider.notifier).addDocument(
                    clientId: client.id,
                    name: docName,
  ...
  void _handleDocumentAction(BuildContext context, WidgetRef ref, String action, ClientDocument document) {
    switch (action) {
      case 'view':
  ...
  void _openDocument(BuildContext context, ClientDocument document) async {
  ...
  void _showDeleteConfirmation(BuildContext context, WidgetRef ref, ClientDocument document) {
    showDialog(
      context: context,
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              Navigator.of(dialogContext).pop();
              final success = await ref.read(clientDocumentsProvider.notifier).deleteDocument(document.id);
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
